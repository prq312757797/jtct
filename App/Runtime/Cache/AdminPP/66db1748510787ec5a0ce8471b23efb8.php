			...
			<div class="span12">
				<div class="widget-box">
					<div class="widget-title">
						...
						<h5>入库单  <span style="color:#ccc;font-size:12px;">商品入库到一级仓柜，一级仓柜的库存会增加</span></h5>
					</div>
		...
		$.post('/adp.php/Warehouse/ajax_goods_info', {
					...
					for(var i = 0; i < obj.length; i++) {
						...
						<table class="table table-bordered table-striped" style="    border-top: 1px solid #ccc;">
							...
							<form action="<?php echo U('Warehouse/run_ruku_add',array('id'=>$info['id'],'action_state'=>'ruku'));?>" method="post" enctype="multipart/form-data" class="form-horizontal" />
							...
							<?php if(is_array($arr)): foreach($arr as $key=>$arr): ?><tr>
								...
								<div class="form-actions"  style=" padding: 6px 50px 4px;">
									<button type="submit" style="float: right;  " class="btn btn-primary">提交入库</button>
									<a href="goods_shenhe?text=入库单&action_state=1" style="float: right; margin-right: 13px; " class="btn btn-primary">待审核入库单<span style="color:red;">（<?php echo ($count_1); ?>）</span></a>
									<a href="goods_shenhe02?text=入库单&action_state=1" style="float: right; margin-right: 13px; " class="btn btn-primary">已审核入库单<span style="color:red;">（<?php echo ($count_2); ?>）</span></a>
								</div>
