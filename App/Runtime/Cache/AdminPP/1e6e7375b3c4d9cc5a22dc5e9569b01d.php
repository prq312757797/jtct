			...
			$("#id_"+$(item).attr("k")+"_"+$("#id_"+$(item).attr("k")).find("option:selected").val()).children("select").attr("name","a100[]")
						...
						<table class="table table-bordered table-striped" style="    border-top: 1px solid #ccc;">
							...
							<?php if(is_array($list)): foreach($list as $k=>$v): ?><tr>
									<td ><input type="text" style="margin-bottom: 5px;" class="w93" readonly value="<?php echo ($v["fenlei_1_title"]); ?>"/></td>
									<td><input type="text" class="w93" readonly value="<?php echo ($v["fenlei_2_title"]); ?>"/></td>
									<td>
										<select name="fenlei_1" class="w100" onchange="action_12(this)"  k="<?php echo ($k); ?>" id="id_<?php echo ($k); ?>">
											<option></option>
											<?php if(is_array($fenlei)): foreach($fenlei as $key=>$v0): ?><option value="<?php echo ($v0["id"]); ?>"><?php echo ($v0["title"]); ?></option><?php endforeach; endif; ?>
										</select>
									</td>
									<td >
										<?php if(is_array($fenlei)): foreach($fenlei as $key=>$v1): ?><div id="id_<?php echo ($k); ?>_<?php echo ($v1["id"]); ?>" class="child_<?php echo ($k); ?>" style="display:none;">

												<select name="" class="w100"  a="<?php echo ($k); ?>_<?php echo ($v1["id"]); ?>">
													<option ></option>
													<?php if(is_array($v1['child'])): foreach($v1['child'] as $key=>$v2): ?><option value="<?php echo ($v2["id"]); ?>"><?php echo ($v2["title"]); ?></option><?php endforeach; endif; ?>
												</select>
									...
									<td>
										<a  href="javascript:;" val="<?php echo ($v["id"]); ?>" text_name="goods_title" class="edit_text"><?php echo ($v["goods_title"]); ?></a>
					 					<input type="text" class="sort_text w40x" id="goods_title_<?php echo ($v["id"]); ?>" value="<?php echo ($v["goods_title"]); ?>" style="display:none;">
										
									</td>
									<td>
										<a  href="javascript:;" val="<?php echo ($v["id"]); ?>" text_name="ktv_guige" class="edit_text"><?php echo ($v["ktv_guige"]); ?></a>
					 					<input type="text" class="sort_text w40x" id="ktv_guige_<?php echo ($v["id"]); ?>" value="<?php echo ($v["ktv_guige"]); ?>" style="display:none;">
									
								
									</td>
									<td>
										<a  href="javascript:;" val="<?php echo ($v["id"]); ?>" text_name="ktv_danwei" class="edit_text"><?php echo ($v["ktv_danwei"]); ?></a>
					 					<input type="text" class="sort_text w40x" id="ktv_danwei_<?php echo ($v["id"]); ?>" value="<?php echo ($v["ktv_danwei"]); ?>" style="display:none;">
									
										
									</td>
									<td><a  href="javascript:;" val="<?php echo ($v["id"]); ?>" key="<?php echo ($k); ?>"  class="goods_save f30" >○</a></td>
									<td><a  href="javascript:;" val="<?php echo ($v["id"]); ?>"  class="goods_delete">X</a></td>
...
//	商品删除
	$(".goods_delete").click(function(){
		...
		if(confirm('确认删除商品吗？')==false){
			...
			$.post('/adp.php/Warehouse/goods_deletektv',{'id':$(that).attr("val")},function(data){
		        if(data==1){
		        	alert("成功");
		        	location.reload()
		        }else{
...
//			'fenlei_2':fenlei_2,
		...
		if($("#choose_"+$(that).attr("key")).find("option:selected").val()){
			data.fenlei_2=$("#choose_"+$(that).attr("key")).find("option:selected").val()
				...
				console.log(data)
		$.post('/adp.php/Warehouse/goods_savektv',data,function(data){
		        if(data==1){
		        	alert("成功");
		        	location.reload()
		        }else{
