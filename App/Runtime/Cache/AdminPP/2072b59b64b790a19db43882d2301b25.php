			...
			<div class="span12">
				<div class="widget-box">
					<div class="widget-title">
						...
						<h5>退库单  <span style="color:#ccc;font-size:12px;">二级仓柜的退商品到一级仓柜，二级仓柜库存减少，一级仓柜库存增加</span></h5>
					</div>
...
//	
	...
	function action_123(item){
		...
		$.post('/adp.php/Warehouse/ajax_goods_info', {
					'fenlei_2': $(item).find("option:selected").val(),'warehouse01':$("#warehouse02_"+key_1).find("option:selected").val()
				}, function(date) {
					var obj = eval(date);
					var dh = '';
					...
					for(var i = 0; i < obj.length; i++) {
						...
						<table class="table table-bordered table-striped" style="    border-top: 1px solid #ccc;">
							...
							<tr>
								...
								<th style="width:10%;">二级仓柜</th>
								<th style="width:10%;">一级仓柜</th>
							...
							<form action="<?php echo U('Warehouse/run_ruku_add',array('id'=>$info['id'],'action_state'=>'tuiku'));?>" method="post" enctype="multipart/form-data" class="form-horizontal" />
							...
							<?php if(is_array($arr)): foreach($arr as $key=>$arr): ?><tr>
									...
									<td><!--一级仓柜-->
										...
										<select name="warehouse01[]" class="w100" id="warehouse01_<?php echo ($arr); ?>" >
											<option ></option>
											<?php if(is_array($warehouse01)): foreach($warehouse01 as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>"><?php echo ($v["title"]); ?></option><?php endforeach; endif; ?>
										</select>
									</td>
									<td><!--大类-->
								...
								<div class="form-actions"  style=" padding: 6px 50px 4px;">
									<button type="submit" style="float: right;  " class="btn btn-primary">提交退库</button>
									<a href="goods_shenhe?text=退库单&action_state=4" style="float: right; margin-right: 13px; " class="btn btn-primary">待审核退库单<span style="color:red;">（<?php echo ($count_1); ?>）</span></a>
									<a href="goods_shenhe02?text=退库单&action_state=4" style="float: right; margin-right: 13px; " class="btn btn-primary">已审核退库单<span style="color:red;">（<?php echo ($count_2); ?>）</span></a>
								</div>
