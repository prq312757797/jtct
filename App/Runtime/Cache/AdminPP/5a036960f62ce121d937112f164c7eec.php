						...
						<table class="table table-bordered table-striped">
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
										<td style="border: 1px;"><div style="width: 184%;font-weight: 600;">订单编号:  <?php echo ($v["order_num"]); ?></div></td>
										<td></td>
										
									</tr>
									<tr>
										<td><img src="/Uploads/<?php echo ($v["goods_info"]["image"]); ?>" style="width:50px;height:50px;border:1px solid #ccc; padding:1px;"></td>
										<td><?php echo ($v["goods_info"]["goods_title"]); ?></td>
										<td><?php echo ($v["price"]); ?></td>
										<td>
											<?php echo (date("Y-m-d",$v["time_build"])); ?><br><?php echo (date("H:i:s",$v["time_build"])); ?>
										</td>
										<td>
									...
									-->
											<a class="btn btn-danger bottom_u" style="color:white;"  title="确认付款" alt="确认付款" href="/adp.php/Orderktvroom/fukuan?id=<?php echo ($vo["id"]); ?>" onclick="if(confirm('确认此订单已付款吗？')==false)return false;">确认付款</a>
