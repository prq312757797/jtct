			...
			<div class="span12">
				<div class="widget-box">
					<div class="widget-title">
						...
						<h5>直拨单  <span style="color:#ccc;font-size:12px;">商品不经过一级仓柜，直接入库到二级仓柜，二级仓柜库存会增加</span></h5>
					</div>
...
//		console.log(111);
		...
		$.post('/adp.php/Warehouse/ajax_goods_info', {
					'fenlei_2': $(item).find("option:selected").val()
				}, function(date) {
					var obj = eval(date);
					var dh = '';
					...
					for(var i = 0; i < obj.length; i++) {
						...
						<table class="table table-bordered table-striped" style="    border-top: 1px solid #ccc;">
							...
							<form action="<?php echo U('Warehouse/run_ruku_add',array('id'=>$info['id'],'action_state'=>'zhibo'));?>" method="post" enctype="multipart/form-data" class="form-horizontal" />
							...
							<?php if(is_array($arr)): foreach($arr as $key=>$arr): ?><tr>
									...
									<td><!--供货商-->
										...
										<select name="supplier[]" class="w100" >
											<option ></option>
											<?php if(is_array($supplier)): foreach($supplier as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>"><?php echo ($v["title"]); ?></option><?php endforeach; endif; ?>
										</select>
									</td>
									<td><!--大类-->
								...
								<div class="form-actions"  style=" padding: 6px 50px 4px;">
									<button type="submit" style="float: right;  " class="btn btn-primary">提交直拨单</button>
									<a href="goods_shenhe?text=直拨单&action_state=6" style="float: right; margin-right: 13px; " class="btn btn-primary">待审核直拨单<span style="color:red;">（<?php echo ($count_1); ?>）</span></a>
									<a href="goods_shenhe02?text=直拨单&action_state=6" style="float: right; margin-right: 13px; " class="btn btn-primary">已审核直拨单<span style="color:red;">（<?php echo ($count_2); ?>）</span></a>
								</div>
