		...
		$.post('/adp.php/Warehouse/ajax_goods_info', {
					...
					for(var i = 0; i < obj.length; i++) {
						...
						<table class="table table-bordered table-striped" style="    border-top: 1px solid #ccc;">
							...
							<form action="<?php echo U('Warehouse/run_ruku_add',array('id'=>$info['id'],'action_state'=>'tuihuo'));?>" method="post" enctype="multipart/form-data" class="form-horizontal" />
							...
							<?php if(is_array($arr)): foreach($arr as $key=>$arr): ?><tr>
									...
									<td><!--一级仓-->
										<select name="warehouse[]" class="w100" id="warehouse01_<?php echo ($arr); ?>">
											<option ></option>
								...
								<div class="form-actions"  style=" padding: 6px 50px 4px;">
									...
									<a href="goods_shenhe?text=退货单&action_state=5" style="float: right; margin-right: 13px; " class="btn btn-primary">待审核退货单<span style="color:red;">（<?php echo ($count_1); ?>）</span></a>
									<a href="goods_shenhe02?text=退货单&action_state=5" style="float: right; margin-right: 13px; " class="btn btn-primary">已审核退货单<span style="color:red;">（<?php echo ($count_2); ?>）</span></a>
								</div>
