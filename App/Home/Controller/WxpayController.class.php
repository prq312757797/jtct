...
class WxpayController extends Controller{
	...
	function get_user_info(){
        ...
        //  'js_code'    => "071NcV8N0mw8w62Tyy7N0Xjc9N0NcV8k",
			...
			$params = array(
		//  'appid'      => "wx46ef97074b113fae",
        //	'secret'  	 => "4465b29e86e612b243b9f493974064ba",	
            'appid'      => $qianyue['appid'],
	...
	function save_man_info(){
		...
		if(M("members")->where("openid='".$_GET["openid"]."'"."and program_id='".$_GET["program_id"]."'")->save($dat)){
...
//珠宝小程序						
						...
						if($members["level_id"]==2){
//						注册会员才能有消费金额的累计
							M("members")->where("program_id='".$group_order["program_id"]."'"."and openid='".$group_order["openid"]."'")->setInc("member_total_pay",$post_data["total_fee"]*0.01);
						}
						if($members["member_total_pay"]+$post_data["total_fee"]*0.01<500){
						
						}else{
							$members_save["level_id"]=3;
...
//百业联盟，判断是否有积分消费，执行扣除积分操作	 执行使用消费券操作
					...
					if($group_order["price_dikou_id"]!=0){
						...
						$save_rebate_record["order_num"]=$group_order["group_order"];
...
//				分销佣金发放
				...
				$date["time_pay"]=time();
				$date["real_price"]=$post_data["total_fee"]*0.01;
				$a=M("room_orderbooking")->field("id,openid,price,state,fx_uid,dfx_price01,dfx_price02,dfx_price03")->where("order_num='".$post_data["out_trade_no"]."'")->find();
				if($a["state"]==0){
...
//					M("members")->where("openid='".$a["openid"]."'")->setInc('integral',$a["price"]);
	    	...
	    	}else if($state[1]=="artical_pay"){
	    		
				$date["state"]="1";
				$date["time_pay"]=time();
				$date["real_price"]=$post_data["total_fee"]*0.01;
				$a=M("artical_order")->field("id,openid,price,state,artical_id")->where("order_num='".$post_data["out_trade_no"]."'")->find();	
				
				$save["artical_id"]=$a["artical_id"];
				if($a["state"]==0){
					M("artical_order")->where("order_num='".$post_data["out_trade_no"]."'")->save($date);
					M("members")->where("openid='".$a["openid"]."'")->save($save);
				}

	    	}else if($state[1]=="gold_silver"){
//珠宝购买金银收藏	    		
				$date["state"]="1";
				$date["time_pay"]=time();
				$date["real_price"]=$post_data["total_fee"]*0.01;
				$a=M("record_shoucang")->field("id,openid,price,state,artical_id")->where("order_num='".$post_data["out_trade_no"]."'")->find();	

				if($a["state"]==0){
					M("record_shoucang")->where("order_num='".$post_data["out_trade_no"]."'")->save($date);
					if($a["goods_id"]==1){
	    	...
	    	}else if($state[1]=="ktv_xuzhong"){

                $date["state"]="1";
                $date["time_pay"]=time();
                $date["real_price"]=$post_data["total_fee"]*0.01;
                $a=M("room_xz_order")->field("id,state,order_num_room_order,xuzhong_houre")->where("order_num='".$post_data["out_trade_no"]."'")->find();
                ...
                if($a["state"]==0){
                    M("room_xz_order")->where("order_num='".$post_data["out_trade_no"]."'")->save($date);
