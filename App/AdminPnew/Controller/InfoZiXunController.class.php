						 ...
						 if(empty($_GET['id'])) {
								...
								$receive['fenlei01']="0";
									     if(M('jt03_zx_info')->add($receive)){
									    	if($_COOKIE["program_id"]=="JT201710160953394557"){
										  		$this->success('添加成功1',U('good_news'));
										  	}else{
										  		$this->success('添加成功2',U('info_list'));
										  	}
									    }else{
									    	$this->error('添加失败');	
									    }    
						        ...
						        //执行修改操作==========================================	
						         	...
						         	if(!empty($old_info["image"])){
									  ...
									  if(M('jt03_zx_info')->where(array('id'=>$_GET['id']))->save($receive)){
										  	if($_COOKIE["program_id"]=="JT201710160953394557"){
										  		$this->success('修改成功1',U('good_news'));
										  	}else{
										  		$this->success('修改成功2',U('info_list'));
										  	}
