...
//当前小程序的所属商户入口
//商城版那个样子的多商户主页参数接口    （不同页面参数不一样了）
	public function dsh_index(){
		
	//需要小程序id，  子商户id
	//得到商品列表，logo，name，收藏数，销量数，商品name、image,单商品销量
...
//申请小程序
	public function dsh_add(){
		...
		//商户名称、主营项目、商户介绍、联系人、电话
		//sh_name  main_project   introduce   linkman  tel
	...
	//1230  多商户申请页面 多商户申请需要判断是不是要付费
	function dsh_chech_pay(){
		$a=M("store")->where("program_id='".$_GET["program_id"]."'")->find();
...
function  discuss_delete(){
	...
	function dsh_list(){
		...
		$where["program_id"]=$_GET["program_id"];
		$where["openid"]	=$_GET["openid"];
		$field="id,sh_name,time_register,state,time_register_sayno";
		
		$sql  =" SELECT ".$field." FROM user_info_dsh ";
		$sql .=" WHERE program_id ='".$_GET["program_id"]."'";
		$sql .=" and (openid ='".$_GET['openid']."'"."or openid_manager='".$_GET['openid']."'".")";
...
//	$field='
//		assistant_add.id,assistant_add.name,assistant_add.tel,assistant_add.time_registe,assistant_add.num,
...
//	->select();
	...
	$sql="SELECT assistant_add.id,assistant_add.name,assistant_add.tel,assistant_add.time_registe,assistant_add.num,
		time_stop,assistant_add.time_agree,assistant_add.shop_id,assistant_add.state,
(case when assistant_add.num is not null then assistant_add.num else 0 end) as ass_num 

FROM `assistant_add` 

WHERE assistant_add.program_id = '".$_GET["program_id"]."'";
$order_form = M("assistant_add") -> query($sql);
	...
	function assistant_say_yes(){
		...
		$a=M("assistant_add")->where("program_id='".$_GET["program_id"]."'"." and id='".$_GET["id"]."'")->find();
		$b=M("assistant_add")->where("program_id='".$_GET["program_id"]."'"." and num='".$_GET["num"]."'")->find();
		if(!empty($b)){
			$this->ajaxReturn(array("state"=>-2,"msg"=>"工号重复"));
		}
		if(M("assistant_add")->where("program_id='".$_GET["program_id"]."'"." and id='".$_GET["id"]."'")->save($save)){
			if($_GET["state"]=="yes"){
				$save02["is_ktv_assistant"]=1;
				M("members")->where("program_id='".$a["program_id"]."'"."and openid='".$a["openid"]."'")->save($save02);
			}
