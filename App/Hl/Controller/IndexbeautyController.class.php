...
class IndexbeautyController extends CommonController {
...
//	商品模块   0
	 function goods_commem_0127($str,$goods_state,$str_f){
        ...
        $sql = "select * from h_goods  where  attribute_style02=2 and  program_id='".$_COOKIE["program_id"]."'";
        if(isset($str)){
...
//1128...14.15  增加了新的模块：商品规格-----先把东西存入规格表，然后根据页面传递过来的唯一标识参数，找到那个id，然后写入商品的表里面
	...
	public function fenlei(){
		...
		->where("dsh_id=0 and attribute_style=1 and attribute_style02=2 and program_id='".$_COOKIE["program_id"]."'" )
		->select();
		$all_fenlei=$this->fenlei=$this->bq($fl);
