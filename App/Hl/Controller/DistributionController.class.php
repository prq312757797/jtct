...
class DistributionController extends CommonController {
    ...
    $field='
    	...
    	members.fx_state,members.fx_uid,members.realname,members.tel,
...
//  $list = D("members") -> query($sql);
	...
	$sql_03  =" SELECT  members.id as members_id,members.fx_uid FROM members WHERE  members.fx_state != 0  and members.program_id ='".$_COOKIE["program_id"]."'";
	$list03 = D("members") -> query($sql_03);

//	$all=$this->bq($list,0,4);//递归求得所有子级元素
	
	foreach($list as $k=>$v){
			$sum=$this->getTree($list03,$v['members_id'],3);
	...
	if($_COOKIE["program_id"]=="JT201803151807521312"){
		...
		for($i=0;$i<count($list);$i++){
			if($list[$i]["fx_uid"]!=0){
				$memberss[$i]=M("members")->field("nickname")->where("id='".$list[$i]["fx_uid"]."'")->find();
				$list[$i]["up_nickname"]=$memberss[$i]["nickname"];
			}
		}
...
echo  1;
    ...
    public function fx_lever_edit(){
    	...
    	$a= $this->info=M('fx_lever')->where("id=".$_GET['id'])->find();
...
dump(M('order')->_sql());
 ...
 function get_orderlist_n($state='',$dfx_id){
	...
	//	$sql  =" SELECT  ".$field;
		...
		$sql .=" WHERE  order_form.dfx_id != 0 AND order_form.program_id = members.program_id and order_form.program_id ='".$_COOKIE["program_id"]."'";
...
function cash_del(){
   ...
   $list= $this->list=M("fx_goods_buy")->where("program_id='".$_COOKIE["program_id"]."'")->select();
	...
	function del_dfx_identy(){
		
		$a=M("members")->field("fx_uid")->where("id='".$_GET["id"]."'")->find();
		
		$save["fx_state"]	=0;
		$save["fx_uid"]		=0;
		$save["fx_lever_id"]=0;
		$save["fx_time_registe"]		='';
		$save["fx_time_agree"]		='';
		$save["fx_time_change"]		='';
		$save["fx_change_num"]		='0';
		
		$save["fx_uid_old"]	=$a["fx_uid"];
		$save["time_qx"]	=time();
...
//分销更换操作
	function sure_change(){
		$a=M("members")->field("fx_change_num")->where("id='".$_GET["to_id"]."'")->find();
		
		$save["fx_uid"]	=$_GET["id"];
		$save["fx_time_change"]	=time();
		$save["fx_change_num"]	=$a["fx_change_num"]+1;
		
		if(M("members")->where("id='".$_GET["to_id"]."'")->save($save)) {
            $this->success('操作成功',U('index'));
        }else {
            $this->error('操作失败');
        }
