...
//		$this->pages=$a["pagelist"];
		...
		$a=M("user_info_dsh")->field($field)->where("is_recommend=1 and program_id='".$_COOKIE["program_id"]."'")->select();
...
//申请状态（1：待审核、2：审核通过（待打款）、3：审核拒绝、4：已打款、5：未知错误）    
    ...
    function tixian($state){
		...
		$where["members.program_id"]=$_COOKIE["program_id"];
...
function cash_del(){
    ...
    function get_orderlist_new($state,$str){
		$field='
		user_info_dsh.id,user_info_dsh.sort,user_info_dsh.sh_name,user_info_dsh.linkman,user_info_dsh.tel,
		user_info_dsh.time_add,user_info_dsh.time_over,user_info_dsh.state,user_info_dsh.main_project,user_info_dsh.is_recommend,
		user_info_dsh.openid,user_info_dsh.openid_manager,user_info_dsh.time_register,user_info_dsh.coupon_dsh_num,user_info_dsh.coupon_dsh_num_done,
		
		(case when user_info_dsh.openid is not null then b.nickname else -1 end) as name_1,
		(case when user_info_dsh.openid_manager is not null then c.nickname else -2 end) as name_2

		';
		$sql  =" SELECT  ".$field;
		$sql .=" FROM user_info_dsh ";
		$sql .=" left join members as b ON b.openid=user_info_dsh.openid ";
		$sql .=" left join members as c ON c.openid=user_info_dsh.openid_manager ";
		$sql .=" WHERE user_info_dsh.program_id ='".$_COOKIE["program_id"]."'";
		$sql .=" and user_info_dsh.state = '".$state."'";
...
//			M("record_total")
			...
			for($i=0;$i<count($list);$i++){
				$b[$i]=M("user_info_dsh")->field("sh_name")->where("order_num='".$list[$i]["order_num"]."'")->find();
...
//		dump($sql_01);dump($sql_02);die;
		...
		function manager_set(){
			...
			if(M("user_info_dsh")->where("program_id='".$_COOKIE["program_id"]."'"."and id='".$_GET["shop_id"]."'")->save($save)){
