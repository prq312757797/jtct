...
class GoodsavecController extends CommonController {
	...
	function goods_commem_0127($str,$goods_state,$str_f){
		...
		$where["by_onlinebear"]=2;
        $total = D("goods")->where($where)->count();
        $per = 10;
      
        $page = new \Component\Page($total, $per); 
        $sql = "select * from goods  where by_onlinebear=2 and  program_id='".$_COOKIE["program_id"]."'";
        if(isset($str)){
	...
	public function fenlei(){
		...
		->where("dsh_id=0 and attribute_style=5 and program_id='".$_COOKIE["program_id"]."'" )
		->order("sort")
		->select();
...
//     }   
	...
	public function goods_add(){
		...
		->field('id,uid,title')
		->where("uid=0 and attribute_style=5 and dsh_id=0 and program_id='".$_COOKIE["program_id"]."'")->select();
...
//二级分类名
		...
		$this->fenlei=M("attribute")
		->field('id,uid,title')
		->where("uid=0 and is_show=1 and attribute_style=5 and program_id='".$_COOKIE["program_id"]."'")->select();
...
//处理批发价=============
		...
		if($_POST["fenlei_1_title"]){
			$fenlei_1=M("attribute")->where("uid=0 and attribute_style=5 and title='".$_POST["fenlei_1_title"]."'"."and program_id='".$_COOKIE["program_id"]."'")->find();
