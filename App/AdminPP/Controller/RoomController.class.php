...
class RoomController extends CommonController {
    ...
    public function index(){
        ...
        for($i=0;$i<count($a);$i++){
            ...
            if($a[$i]["state"]==1){
                $d[$i]=M("room_orderbooking")->where("state=2 and room_num='".$a[$i]["room_num"]."'")->find();

                $a[$i]["shengyu_time"]=$d[$i]["time_done"]-time();
    ...
    public function ajax_index(){
        ...
        for($i=0;$i<count($a);$i++){
            ...
            $a[$i]["time"]=date("Y-m-d H:i:s",time());
//0:空闲，1：使用中、2：清台中、3：暂停使用
            if($a[$i]["state"]==0){
                ...
                $a[$i]["shengyu_time"]=$d[$i]["time_done"]-time();
                if($a[$i]["shengyu_time"]>0){
                    $a[$i]["state_text"]="使用中";
                    $a[$i]["color"]="red";
                }else{
                    $a[$i]["shengyu_time"]=0;
                    $save[$i]["state"]="5";
                    M("room_orderbooking")->where("id='".$d[$i]["id"]."'")->save($save[$i]);

                    $save02[$i]["state"]="2";
                    M("room_manager")->where("id='".$a[$i]["id"]."'")->save($save02[$i]);
                    $a[$i]["state_text"]="清台中";
                    $a[$i]["color"]="red";
                }
...
//	清台完成
	function ajax_room_toused(){
        $save["state"]=0;
	    if(M("room_manager")->where("id='".$_POST["id"]."'")->save($save)){
            $a=1;
        }else{
            $a=0;
        }
...
//		    		$data=array("status" => -1, "msg" => "图片上传失败",'error'=>$error);
		    	...
		    	$receive["room_check_code"]="https://sz800800.cn/pg.php/Qdcode/room_check_code?program_id=JT201808090855119076&room_num=".$receive['room_num'];// 房台使用二维码
		    	$receive["onlinebeer_code"]="https://sz800800.cn/pg.php/Qdcode/onlinebeer_code?program_id=JT201808090855119076&room_num=".$receive['room_num'];//在线酒水二维码
		    	$receive["savebeer_code"]="https://sz800800.cn/pg.php/Qdcode/savebeer_code?program_id=JT201808090855119076&room_num=".$receive['room_num'];//存酒二维码
