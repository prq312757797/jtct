...
/*	$user = A('Index');
    ...
    */
		...
		//待审核商户
		$a2=$this->order_form_2=M('user_info_dsh')->where("state=1 and program_id='".$_COOKIE["program_id"]."'")->count();
		
		//已拒绝商户
		$a3=$this->order_form_5=M('user_info_dsh')->where("state=4 and program_id='".$_COOKIE["program_id"]."'")->count();
...
//服务版=============================			
		...
		$fl=M("attribute")
		->field('id,uid,title,image_1,is_show,attribute_style')
...
//		$all_fenlei=$this->fenlei=$this->bq($fl);
		...
		for($i628=0;$i628<count($all_fenlei);$i628++){
			if($all_fenlei[$i628]["attribute_style"]==1){
				//商品分类
				$all_fenlei_1[]=$all_fenlei[$i628];
			}else if($all_fenlei[$i628]["attribute_style"]==2){
				//子商户分类
				$all_fenlei_2[]=$all_fenlei[$i628];
			}
		}
		$this->fenlei=$all_fenlei_1;
		$this->fenlei_dsh=$all_fenlei_2;
