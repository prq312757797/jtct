...
//1:待付款,2：已付款待分配，3：待服务、4：服务完成，5：已取消======================================================================================
	function order_com($state){
		
		$sql  =" SELECT * ";
		$sql .=" FROM car_order ";
		$sql .=" WHERE  order_num !='0' and program_id ='".$_COOKIE["program_id"]."'";
		$sql .=" and state ='".$state."'";
		if($state==1) $sql .=" ORDER BY time_xd desc ";
		if($state==2) $sql .=" ORDER BY time_pay desc ";
		if($state==3) $sql .=" ORDER BY time_pay desc ";
		if($state==4) $sql .=" ORDER BY time_pay desc ";
		if($state==5) $sql .=" ORDER BY time_xd desc ";
...
//待付款	//1:待付款,2：已付款待分配，3：待服务、4：服务完成，5：已取消======================================================================================
...
//已付款待分配 //1:待付款,2：已付款待分配，3：待服务、4：服务完成，5：已取消======================================================================================
...
//待服务 //1:待付款,2：已付款待分配，3：待服务、4：服务完成，5：已取消======================================================================================
public function order_serving(){
		$count=$this->count_tk();
	$this->tk_count=$count;
	$data=$this->order_com($state=3);
	$this ->pages = $data[0];
	$this ->list  = $data[1];
...
//已完成 //1:待付款,2：已付款待分配，3：待服务、4：服务完成，5：已取消======================================================================================
...
//	$this ->list  =$a;
	...
	//已取消 //1:待付款,2：已付款待分配，3：待服务、4：服务完成，5：已取消======================================================================================
...
//	$sql  =" SELECT * ";
//	$sql .=" FROM car_order ";
//	$sql .=" WHERE state=3 and order_num !='0' and program_id ='".$_COOKIE["program_id"]."'";
//	$sql .=" ORDER BY time_xd desc ";
//	
//	$a = M("car_order") -> query($sql);
//
...
//	$this ->list  =$a;
	...
	for($i=0;$i<count($data[1]);$i++){
		if($data[1][$i]["state_tuikuan"]==2) $count+=1;
	}
...
function order_con(){
	...
	$c=M("car_fenlei")->where("id='".$b["fenlei_id"]."'")->find();
	$b_arr=explode(",",$b["image"]);
	$a["car_image"]=$b_arr[0];//车型图片
	$a["car_title"]=$b["title"];//车型名称
	$a["car_fenlei"]=$c["title"];//车型名称
