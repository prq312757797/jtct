...
class GoodsController extends CommonController {
	...
	function goods_tuisong(){
		
		$goods=M("goods")->where("state_area_re=1 and goods_state=3 and program_id='".$_COOKIE["program_id"]."'")->order("time_tuisong")->select();
		for($i=0;$i<count($goods);$i++){
			$user_info_dsh=M("user_info_dsh")->where("id='".$goods[$i]["dsh_id"]."'")->find();

			$goods[$i]["sh_name"]=$user_info_dsh["sh_name"];
		}
		
		$this->goods=$goods;
		$this->display();
	}
	function tuisong_yes(){
		$save["state_area_re"]=0;
		$save["goods_state"]=1;
	
		$a=M("goods")->where("id='".$_POST["id"]."'")->find();
		if(M("goods")->where("id='".$_POST["id"]."'")->save($save)) {
			$add["goods_id"]=$_POST["id"];
			$add["dsh_id"]=$a["dsh_id"];
			$add["time_on"]=$a["time_tuisong"];
			$add["time_yes"]=time();
			$add["program_id"]=$_COOKIE["program_id"];
			M("goods_tuisong_record")->add($add);
           $state=1;
        } else {
            $state=-1;
        }
	...
	function tuisong_no(){
		$save["state_area_re"]=0;
		$save["goods_state"]=2;
		$save["text_sayno"]=$_POST["text_sayno"];
		if(M("goods")->where("id='".$_POST["id"]."'")->save($save)) {
			$add["goods_id"]=$_POST["id"];
			$add["dsh_id"]=$a["dsh_id"];
			$add["time_on"]=$a["time_tuisong"];
			$add["time_no"]=time();
			$add["program_id"]=$_COOKIE["program_id"];
			$add["text_sayno"]=$_POST["text_sayno"];
			M("goods_tuisong_record")->add($add);
             $state=1;
        } else {
            $state=-1;
	...
	function goods_commem_0127($str,$goods_state,$str_f){
        ...
        $sql = "select * from goods  where goods_baiye=0 and by_onlinebear=0 and program_id='".$_COOKIE["program_id"]."'";
        if(isset($str)){
...
//		$save["title"]=222;
//		M("test")->where("id=4564")->save($save);
//		dump(M("test")->_sql());die;
	...
	public function fenlei(){
		...
		$fl=M("attribute")
		->field('id,uid,title,image_1,is_show,title_english,cannot_edit')
...
//		珠宝小程序金饰银饰分类
	...
	public function fenlei_zhekou(){

		$this->fenlei=$fl=M("attribute_zhekou")
		->where(" program_id='".$_COOKIE["program_id"]."'" )
		->select();
	...
	public function fenlei_zhekou_add(){
		
		$this->display();
	}

	
	public function fenlei_zhekou_edit(){
		 $a= $this->info=M('attribute_zhekou')->where("id=".$_GET['id'])->find();
	
		$this->display('fenlei_zhekou_add');
	}
	
	public function run_fenlei_zhekou_add(){
		$receive=I();
		$receive["program_id"]=$_COOKIE["program_id"];
			$upload = new \Think\Upload();// 实例化上传类
			    $upload->maxSize   =     3145728 ;// 设置附件上传大小
			    ...
			    $info   =   $upload->upload();
			     ...
			     if(empty($_GET['id'])) {
				    ...
				    if(M('attribute_zhekou')->add($receive)){
				    	$this->success('添加成功.',U('fenlei_zhekou'));
				    }else{
				    	$this->error('添加失败.');	
				    }
			     }else{
			     	if(M('attribute_zhekou')->where(array('id'=>$_GET['id']))->save($receive)){

						  $this->success('修改成功..',U('fenlei_zhekou'));
				    }else{
				    	$this->error('修改失败..');	
				    }
				 }
	...
	//分类删除
		public function fenlei_zhekou_delete(){
			$user_id =$_GET['id'];
			
	        if(M('attribute_zhekou')->delete($user_id)) {
	            $this->success('删除成功');
	        } else {
	            $this->error('删除失败');
	        }
	...
	public function goods_add(){
		 ...
		 if($_COOKIE["program_id"]=="JT201810111844206577"){
		 	 $this->fenlei_zhekou=M("attribute_zhekou")->where("program_id='".$_COOKIE["program_id"]."'")->select();  
		 }
		...
		function json_decde_cl($json_data){
...
//			$json=json_decode($info["json_price_cl"],true);
//			$key=0;
//			foreach($json as $k=>$v){
//				$arr1[$key]=$k;
//				$arr2[$key]=$v;
//				$key++;
//			}
			$return_data=$this->json_decde_cl($json_data=$info["json_price_cl"]);
			$arr1=$return_data[1];
			$arr2=$return_data[2];
		}
		...
		$attribute_zhekou=M("attribute_zhekou")->where("id='".$info["fenlei_zhekou"]."'")->find();
		$info["fenlei_zhekou_title"]=$attribute_zhekou["title"];
...
//二级分类名
	...
	if(!empty($info["str_guige"])){
		...
		$arr1128_7=explode('&',$info["guige_json_price_cl"]);
		...
		for($q1=0;$q1<count($arr1128_1);$q1++){
			...
			$return_data[$q1]=$this->json_decde_cl($json_data=$arr1128_7[$q1]);
		
			
			$arr1128[$q1]["json_cl"]["cl_arr_title"]=$return_data[$q1][1];
			$arr1128[$q1]["json_cl"]["cl_arr_price"]=$return_data[$q1][2];
		}	
	...
	if($_COOKIE["program_id"]=="JT201807260907385777"){
		$this->video=M("video")->where("goods_id='".$_GET['id']."'")->select();
		$this->goods_id=$_GET['id'];
...
//		绑定的在线酒水库存		
		$tie_goods=M("goods")->field("goods_title")->where("id='".$info["ktv_tie_goods_id"]."'")->find();
		$info["tie_goods_title"]=$tie_goods["goods_title"];
...
//二级分类名
	...
	public function ajax_fenlei_2(){
		...
		//异步获得子类
	public function ajax_fenlei_202(){

		$b=M("attribute")->where("uid='".$_POST["title"]."'")->select();
		 for($i=0;$i<count($b);$i++){
            $arr_road[$i]["title"]=$b[$i]["title"];
			$arr_road[$i]["id"]=$b[$i]["id"];
        }
		$this->ajaxReturn($arr_road);
	}
	public function run_goods_add(){

//			能量ktv房台套餐商品  绑定在线酒水商品库存 fenlei_2_title
		if($_COOKIE["program_id"]=="JT201808090855119076"){
			
			
			if($_POST["tie_goods_id"]){
				$_POST["ktv_tie_goods_id"]=$_POST["tie_goods_id"];
				$_POST["ktv_tie_goods_num"]=$_POST["tie_goods_num"];
			}
...
//处理批发价=============
		...
		if($_POST["fenlei_1_title"]){
			...
			if($fenlei_1["id"]!=1422 & $_POST["program_id"]=="JT201808090855119076"){
				$this->error('分类选择错误');	
			}
...
//		$_POST["goods_subtitle05"]=(empty($_POST["good_attribute05"]))?2:1;
		...
		if($_COOKIE["program_id"]=="JT201810111844206577"){
//		无规格但商品材料价格
			for($i=0;$i<count($_POST["price_cl_title"]);$i++){
				if($_POST["price_cl_title"][$i]){
					$arr[$_POST["price_cl_title"][$i]]=$_POST["price_cl"][$i];
				}	
			}
			$json=json_encode($arr);	
				
			$_POST["json_price_cl"]=	$json;
		
//			规格商品材料价格
			
			for($gg_key=0;$gg_key<10;$gg_key++){
				if(count($_POST["gg_price_cl_title_to_".$gg_key])>1){
					$value[$gg_key]=$this->json_cl($arr_title=$_POST["gg_price_cl_title_to_".$gg_key],$arr_price=$_POST["gg_price_cl_to_".$gg_key]);
					if(empty($_POST["guige_json_price_cl"])){
						$_POST["guige_json_price_cl"]=$value[$gg_key];
					}else{
						$_POST["guige_json_price_cl"].="&".$value[$gg_key];
					}
//					if(empty($str_value[$gg_key])){
//						$str_value=$value[$gg_key];
//					}else{
//						$str_value.="&".$value[$gg_key];
//					}
...
//	$dd=json_decode($_POST["guige_json_price_cl"],true);
//	dump($_POST["guige_json_price_cl"]);dump($dd);die;
		}
...
//		dump($_POST["price_cl_title"]);dump($_POST["price_cl"]);dump($arr);dump($json);dump($arr2);die;		
...
//1128...14.15  增加了新的模块：商品规格-----先把东西存入规格表，然后根据页面传递过来的唯一标识参数，找到那个id，然后写入商品的表里面
		...
		function json_cl($arr_title,$arr_price){
			for($i=0;$i<count($arr_title);$i++){
				if($arr_title[$i]){
					$arr[$arr_title[$i]]=$arr_price[$i];
				}	
			}
			$json=json_encode($arr);
			return $json;
	...
	function ajax_change_ok_2(){
	$state=M("goods")->where("id='".$_POST["id"]."'")->find();
			if($state["index_choose"]==1){
				$val["index_choose"]="0";	
			}else if($state["index_choose"]==0){
				$val["index_choose"]="1";
			}
			if(M("goods")->where("id='".$_POST["id"]."'")->save($val)){
					$date="1";
		...
		$receive["program_id"]=$_COOKIE["program_id"];
			$upload = new \Think\Upload();// 实例化上传类
			    ...
			    if($info){
                   ...
                   if($info['videoimage02']){
			    		$receive['videoimage']=$info['videoimage02']['savepath'].$info['videoimage02']['savename'];
			    	}
			    	...
			    	if(empty($_GET['id'])) {
			    		$this->error('无上传视频');	
	                	$error=$upload->getError();
			    		$data=array("status" => -1, "msg" => "视频上传失败",'error'=>$error);
			    	}
