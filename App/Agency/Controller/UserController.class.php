...
class UserController extends Controller {
	...
	//客户管理页面信息列表 
	//需求参数 服务商ID
	//提供参数user_info.realname,qianyue.appmcheng,qianyue.state,qianyue.program_id
	public function user_list(){
		$Mysql=M('user_info');
//	    $id=I('get.id');
		$where['user_info.agency_id']=I('get.id');
		$field='user_info.realname,qianyue.appmcheng,mail,qianyue.state,qianyue.program_id,qianyue.quick_notice,qianyue.mail_notice,jt_user_info.user_company';
		$list=$Mysql
		->field($field)
...
//		$where['user_info.lasttime']=array(array('EGT',$stampt),array('ELT',$nt));
		...
		$field='user_info.realname,qianyue.appmcheng,qianyue.state,qianyue.program_id,qianyue.quick_notice,qianyue.mail_notice,jt_user_info.user_company';
		$list=$Mysql
		->field($field)
		->join('LEFT JOIN qianyue ON user_info.id=qianyue.user_id')
		->join('LEFT JOIN jt_user_info ON user_info.agency_id=jt_user_info.user_id')
		->where($where)
		->select();
		for($i=0;$i<count($list);$i++){
				$list[$i]['xuhao']=$i+1;
